...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (completed || (seconds && seconds > 30)) {
    ...
    if (v && v.user_id !== payload.sub) { const reward = completed ? 2 : 1; const c = store.users.get(v.user_id); if (c) c.nst_balance += reward; store.transactions.set('tx_' + generateId(), { id: 'tx_' + generateId(), user_id: v.user_id, type: 'watch_reward', amount: reward, description: `Watch ${seconds}s no vídeo ${id}`, status: 'completed', created_at: new Date().toISOString() }) }
  }
