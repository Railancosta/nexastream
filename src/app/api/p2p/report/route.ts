...
export async function POST(req: NextRequest) {
  ...
  if (reward > 0) { const user = store.users.get(payload.sub); if (user) user.nst_balance += reward; store.transactions.set('tx_' + generateId(), { id: 'tx_' + generateId(), user_id: payload.sub, type: 'seeding_reward', amount: reward, description: `Seeding vídeo ${video_id}`, status: 'completed', created_at: new Date().toISOString() }) }
